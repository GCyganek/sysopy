...
void pizza_to_furnace() {
    ...
    semop(semaphores_id, operations, 2);
    ...
    semop(semaphores_id, &unlock_furnace, 1);  // we unlock the furnace
    shmdt(p);
}

void pizza_from_furnace() {
    semop(semaphores_id, &lock_furnace, 1);  // wait til the furnace is available and lock it, we take out a pizza from the furnace
...
void pizza_on_table() {
    ...
    semop(semaphores_id, &unlock_table, 1);  // we unlock the table
...
int main() {
    ...
    while(1) {
        ...
        if (semctl(semaphores_id, FURNACE_SPACE_SEMAPHORE, GETVAL, NULL) < FURNACE_CAPACITY) { // it should not happen though, right?
            pizza_from_furnace();
            pizza_on_table();
        }
    }
